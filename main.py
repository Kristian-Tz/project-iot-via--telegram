...
def start_bot(message):
    bot.send(message['message']['chat']['id'], "Halo, Selamat Datang di Smart_Home by KRISTIAN")

def get_message(message):
    if message['message']['text'].lower() == 'hidupkan lampu':
        if relay.value() == 0:
            bot.send(message['message']['chat']['id'],'Lampu Sudah Menyala')
        else:
            relay.value(0)
            bot.send(message['message']['chat']['id'],'Menyalakan Lampu')
    elif message['message']['text'].lower() == 'matikan lampu':
        if relay.value() == 1:
